...
try {
    ...
    if (isset($data["product_detail_ids"]) && is_array($data["product_detail_ids"])) {
        $productDetailIds = $data["product_detail_ids"];
        $successCount = 0;
        $errorCount = 0;

        foreach ($productDetailIds as $productDetailId) {
            $affectedRows = deleteProductDetails($productDetailId); // Gọi hàm xóa chi tiết sản phẩm
        ...
        echo json_encode([
            "status" => "success",
            "message" => "Product details processed",
            "details" => [
        ...
        throw new Exception("product_detail_ids must be a valid array.");
    }
} catch (Exception $e) {
