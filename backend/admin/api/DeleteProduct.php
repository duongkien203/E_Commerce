...
try {
    ...
    if (isset($data["product_id"])) {
        $productId = intval($data["product_id"]); // Lấy ID sản phẩm từ body
        $response = deleteProduct($productId); // Gọi hàm xóa sản phẩm
        echo $response; // Trả về phản hồi từ hàm
    } else {
        throw new Exception("product_id is required in the request body");
    }
} catch (Exception $e) {
