...
const PhonePopup = ({ onClose, accountId }) => {
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await fetch(
          "http://localhost/E_Commerce/backend/user/api/VerifyAccountAndSendOTP.php", // Sử dụng cùng API
          {
            method: "POST",
            credentials: "include",
  ...
  const verifyOtp = async (otp) => {
    try {
      const response = await fetch(
        "http://localhost/E_Commerce/backend/user/api/VerifyOTPBeforeUpdate.php", // Sử dụng cùng API
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
