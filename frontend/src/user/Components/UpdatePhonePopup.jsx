...
const UpdatePhonePopup = ({ onClose, accountId }) => {
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await fetch(
          "http://localhost/E_Commerce/backend/user/api/VerifySendOTP.php",
          {
            method: "POST",
            credentials: "include",
  ...
  const verifyOtp = async (otp) => {
    try {
      const response = await fetch(
        "http://localhost/E_Commerce/backend/user/api/VerifyOTPNewContact.php",
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
    ...
    <div className={styles.overlay}>
      <div className={`${styles.popup} ${styles.fadeIn}`}>
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          {formik.touched.newPhone && formik.errors.newPhone && (
            <div className={styles.errorValidate}>{formik.errors.newPhone}</div>
          )}
